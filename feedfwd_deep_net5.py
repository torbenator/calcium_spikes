...
class FFN_architecture:
    ...
    def mlpnet(self, x,_dropout= 0.9, nn1= 64, nn2=16, nn3=32, num_att =14):
...
def train_neural_network(att_data, clipped_spikes, hm_epochs=4, batch_size= 100):
    ...
    train_x, test_x, train_y, test_spikes = train_test_split(att_data, clipped_spikes, test_size=0.2)    
    model = FFN_architecture()
    print('Model successfully acquired')
    o = model.mlpnet(model.x)
    

    loss = tf.reduce_sum(tf.reduce_mean(tf.abs(tf.sub(o, model.y)), 0)) 
#    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(o,model.y))
#    loss = tf.reduce_mean(tf.contrib.losses.sum_of_squares(o, model.y))
#    loss = tf.contrib.losses.softmax_cross_entropy(o,model.y)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            while i < len(train_x):
                ...
                _, c = sess.run([optimizer, loss], feed_dict={model.x: batch_x, model.y: batch_y})
                epoch_loss += c
                i+=batch_size
            print('Epoch', epoch+1, 'completed out of ', hm_epochs,'loss:',epoch_loss)
            spike_predict= o.eval({model.x:test_x})
            sanity_check = model.y.eval({model.y:test_spikes})
            
    tf.reset_default_graph()
    return test_spikes, spike_predict, sanity_check
